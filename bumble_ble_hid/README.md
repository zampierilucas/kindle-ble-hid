...
```
MediaTek MT8512 → /dev/stpbt → Bumble Stack → Script Execution
                                    |
                                    +---> Multiple devices in parallel
                                    +---> Auto-reconnection
                                    +---> Button → Shell script mapping
```
...
**Setup**: First, discover and configure your BLE devices:

```bash
# Easy way: Use the setup helper script
cd /mnt/us/bumble_ble_hid
./setup_device.sh
```
...
2. Show you all discovered devices with their MAC addresses
3. Let you select which one to add to devices.conf
4. Automatically configure the daemon

**Manual setup** (if you prefer):
```bash
# Scan for devices manually
./kindle_ble_hid.sh --scan-only --scan-duration 30

# Create devices.conf and add device addresses
vi /mnt/us/bumble_ble_hid/devices.conf
```

Example `/mnt/us/bumble_ble_hid/devices.conf`:
```
...
- Button presses trigger configurable shell scripts
- Configuration via `/mnt/us/bumble_ble_hid/button_config.json`
- Configurable debouncing (default 500ms)
- Works with BLE-M3 and similar BLE HID remotes

See `SCRIPT_MODE_README.md` for detailed configuration.
